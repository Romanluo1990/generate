package com.vip.xpf.common.util.bean.orika;

import com.vip.xpf.common.util.bean.orika.annotation.ClassMap;
...
public class LazyMapperFacade extends MapperFacadeImpl {
  ...
  public <S, D> MappingStrategy resolveMappingStrategy(
    ...
    return super.resolveMappingStrategy(
        sourceObject, initialSourceType, initialDestinationType, mapInPlace, context);
  }
        ...
        if (!initCache.contains(descClass)) {
          ...
          if (classMap != null) {
            ...
            Arrays.stream(classMap.mapNullsDisable())
                .forEach(initMapNullsDisable(descClass, descClassMapBuilders));
          }
          // 注册FieldMap
          List<Field> fields = getAllFields(descClass);
          fields.forEach(initFieldMapper(descClass, descClassMapBuilders));
          descClassMapBuilders
              .values()
              .stream()
              .map(ClassMapBuilder::byDefault)
  ...
  private Consumer<Field> initFieldMapper(
      Class<?> classB, Map<Class, ClassMapBuilder> descClassMapBuilders) {
    return field -> {
  ...
  private Consumer<? super FieldMap> addFieldMapBuilder(
      Class<?> classB, String destField, Map<Class, ClassMapBuilder> descClassMapBuilders) {
    return fieldMap -> {
      ...
      FieldMapBuilder fieldMapBuilder =
          classMapBuilder.fieldMap(orgiField, destField, byDefault).mapNulls(mapNulls);
      if (exclude) {
  ...
  private List<Field> getAllFields(Class<?> clazz) {
    ...
    while (tempClass != null
        && !Object.class.equals(tempClass)) { // 当父类为null的时候说明到达了最上层的父类(Object类).
  ...
  private Consumer<Class<?>> initMapNullsDisable(
      Class<?> classB, Map<Class, ClassMapBuilder> descClassMapBuilders) {
    return classA -> {
      ClassMapBuilder classMapBuilder = descClassMapBuilders.get(classA);
      if (classMapBuilder == null) {
